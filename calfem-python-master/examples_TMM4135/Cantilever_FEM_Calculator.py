...
vecNx = np.arange(3,49,2)
vecNy = vecNx

dispmatrix = np.array(np.zeros((len(vecNx), len(vecNy))))

#numElementNodes = 9  # Valid numbers 3, 4, 6, 9
for n in element:
    displacements = np.array([])
    for Ny in range(len(vecNy)):
        for Nx in range(len(vecNx)):
            numNodesX = vecNx[Nx]
            numNodesY = vecNy[Ny]
            numElementNodes = n
            print(vecNx[Nx], vecNy[Ny], n)
    ...
    plt.suptitle(n)
    Xplot,Yplot = np.meshgrid(vecNx,vecNy)
    name = str(n) + str("3d.png")
    ha.plot_surface(Xplot, Yplot, dispmatrix,cmap=cm.coolwarm)
    ha.set_zlim3d(2e-5, 6.3e-5)
    plt.savefig(name)
    plt.show()
    plt.close()
    nodesx = []
